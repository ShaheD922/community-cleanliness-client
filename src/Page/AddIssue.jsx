  ...
  return (
    <div className="p-6 max-w-md mx-auto bg-white rounded-lg shadow">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        <input type="text" placeholder="Issue Title" value={title} onChange={(e) => setTitle(e.target.value)} required className="border p-2 rounded" />
        ...
        <input type="text" placeholder="Location" value={location} onChange={(e) => setLocation(e.target.value)} required className="border p-2 rounded" />
        <textarea placeholder="Description" value={description} onChange={(e) => setDescription(e.target.value)} required className="border p-2 rounded" />
        ...
        <input type="text" placeholder="Suggested Fix Budget (Amount)" value={amount} onChange={(e) => setAmount(e.target.value)} required className="border p-2 rounded" />
        <input type="email" value={user?.email || ""} readOnly className="border p-2 rounded bg-gray-100" />
        <button type="submit" className="bg-green-600 text-white p-2 rounded hover:bg-green-700">Add Issue</button>
      </form>
    </div>
  );
};
