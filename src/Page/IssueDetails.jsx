...
import LoadingPage from "../Components/LoadingPage"; // tomr loading component
...
const IssueDetails = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const contribRes = await fetch(`http://localhost:5000/mycontribution/${id}`);
  ...
  const handleContribution = async (e) => {
    ...
    const contribution = {
      ...
      date: new Date(),
      additionalInfo,
      image: user.photoURL || "", // optional: user image
    };
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      ...
      />
      <div className="mb-1"><strong>Category:</strong> {issue.category}</div>
      <div className="mb-1"><strong>Location:</strong> {issue.location}</div>
      <div className="mb-1"><strong>Description:</strong> {issue.description}</div>
      <div className="mb-1"><strong>Budget:</strong> {issue.amount}</div>
      <div className="mb-4"><strong>Date:</strong> {new Date(issue.date).toLocaleString()}</div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded w-full max-w-md shadow-lg">
            <form onSubmit={handleContribution} className="flex flex-col gap-2">
              ...
              <button type="submit" className="bg-blue-600 text-white p-2 rounded hover:bg-blue-700 transition">Submit</button>
        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {contributors.map((c, idx) => (
              <div
                key={idx}
                className="bg-white p-4 rounded-lg shadow-md hover:shadow-xl transition transform hover:-translate-y-1 flex items-center gap-4"
              >
                <img
                ...
                />
                <div>
                  <p className="font-bold"> Name : {c.contributorName || "Anonymous"}</p>
                  <p className="text-gray-500">Amount : {c.amount}</p>
                </div>
              </div>
            ))}
          </div>
